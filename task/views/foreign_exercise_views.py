...
from english.analytics.foreign_analytics import collect_statistics
from english.models import WordModel, WordUserKnowledgeRelation
from task.forms import ForeignTranslateChoiceForm
...
class ForeignWordTranslateChoiceView(CheckLoginPermissionMixin, TemplateView):
    ...
    template_name = 'task/foreign/foreign_translate_choice.html'
    ...
    def get_context_data(self, **kwargs: object) -> Dict[str, object]:
        ...
        Adds :py:class:`~task.forms.foreign_translate_choice_form.ForeignTranslateChoiceForm`
        """  # noqa: E501, W505
    ...
    def post(self, request: HttpRequest) -> HttpResponse:
        ...
        if form.is_valid():
            ...
            return redirect(reverse_lazy('task:foreign_translate_demo'))
...
class ForeignTranslateExerciseView(CheckLoginPermissionMixin, View):
    ...
    template_name = 'task/foreign/foreign_translate_demo.html'
    """The view template path (`str`).
    """
    ...
    """
    redirect_no_words = {
        'redirect_no_words': reverse_lazy('task:foreign_translate_choice'),
    }

    def get(self, request: HttpRequest) -> HttpResponse:
        """Display an exercise page to translate a word."""
        task_conditions = request.session[TASK_CONDITIONS]
        task = ForeignWordTranslateExercise(**task_conditions)

        try:
            task.create_task()
        except KeyError:
            messages.error(request, self.msg_key_error)
            return redirect(reverse_lazy('task:foreign_translate_choice'))
        except ValueError:
