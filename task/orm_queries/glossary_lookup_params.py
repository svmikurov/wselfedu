...
class GlossaryExerciseLookupParamsSkip:
    ...
    def progress(self) -> Q:
        """Lookup parameter by study progress (`Q`, read-only)."""
        lookup_alice = self.lookup_conditions.get('progress', [])
        lookup_value = self._to_numeric(PROGRESS_STAGE_EDGES, lookup_alice)
        lookup_field = 'progress'
        param = Q(**{lookup_field: lookup_value}) if lookup_value else Q()
        return param
    ...
    @staticmethod
    def _to_numeric(assessments: dict, string_values: list) -> list[int]:
        """Convert a literal representation of a progress.
