...
function getDemoTask () {
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#question').attr("href", data.term_detail_link)
            $('#question_text').text(data.question_text);
            $('#answer_text').hide();
            $('#stub').show();
            $('#answer_text').text(data.answer_text);
            // Set task info
            $('#term_count').text('Выбрано терминов: ' + data.term_count);
            $('#progress').text('Уровень: ' + data.progress);
        ...
        error: function (data) {
            window.location.href = data.responseJSON.redirect_no_terms;
        }
    })
}
