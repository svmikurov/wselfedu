 ...
 * 
 * ⚡ HOW TO ENABLE LOGS IN PRODUCTION:
...
window.appLogger = {
     ...
     * Includes timestamp
     */
    log: (...args) => (window._forceLogs || isLocal) && console.log(`[${getTimestamp()}]`, '[LOG]', ...args),
     ...
     * Includes timestamp
     */
    debug: (...args) => (window._forceLogs || isLocal) && console.debug(`[${getTimestamp()}]`, '[DEBUG]', ...args),
     ...
     * Includes timestamp
     */
    info: (...args) => (window._forceLogs || isLocal) && console.info(`[${getTimestamp()}]`, '[INFO]', ...args),
     ...
     * Includes timestamp
     */
    warn: (...args) => (window._forceLogs || isLocal) && console.warn(`[${getTimestamp()}]`, '[WARN]', ...args),
     ...
     */
    group: (...args) => (window._forceLogs || isLocal) && console.group(`[${getTimestamp()}]`, ...args),
    groupEnd: () => (window._forceLogs || isLocal) && console.groupEnd(),
    groupCollapsed: (...args) => (window._forceLogs || isLocal) && console.groupCollapsed(`[${getTimestamp()}]`, ...args),
    ...
    enable: (force = null) => {
        if (force !== null) {
            ...
            console.log(`[${getTimestamp()}] 🔍 Logging ${force ? 'ENABLED' : 'DISABLED'}`);
        }
    ...
    time: (label) => {
        ...
        return () => {
            ...
            if (window._forceLogs || isLocal) {
                console.log(`[${getTimestamp()}] ⏱️ ${label}: ${duration.toFixed(2)}ms`);
            }
        };
    }
};
